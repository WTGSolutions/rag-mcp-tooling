...
export function chunkYaml(
  ...
  if (starts[0]! > 0) {
    chunks.push(...windowTrimmedSpan(lines.slice(0, starts[0]!), 1, file, config, fileHash));
  }
  ...
  for (let h = 0; h < anchors.length; h++) {
    ...
    if (estimateTokens(sectionText) <= config.maxTokens) {
      chunks.push(createChunk({
        file, fileHash,
        startLine: sStart + 1, endLine: sEnd + 1,
        text: sectionText, kind: 'block', symbol,
      }));
      continue;
    }

    // Oversized block → window by token budget; every window keeps the key symbol.
    for (const { startIdx, endIdx } of windowLineRanges(sectionLines, config.maxTokens, config.overlapLines)) {
      chunks.push(createChunk({
        file, fileHash,
        startLine: sStart + startIdx + 1, endLine: sStart + endIdx,
